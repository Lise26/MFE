...
% for j="corr_cc"
    ...
    for i=1:nb_patients
        ...
        par(c) = predict(file_name, 8, j, "test");
        results(i, :) = [par(c).Av_density, par(c).Av_clustering_coeff, par(c).Av_char_path_length, par(c).Av_size_larg_comp, par(c).Av_char_path_length_lc, par(c).Av_nb_ind_comp];
...
hold on
plot(par(3).Char_path_length_lc)
xlabel('Time (in ms)')
